...
- Event emission: events publish from the boundary that owns the transaction — the route handler for HTTP, the worker task body for timers. Logic functions in `core/logic/` must NOT call `emit()`/`emit_gameplay()` themselves; they return events for the caller to emit. The default `emit()`/`emit_gameplay()` are after-commit-safe (buffer until `Session.commit()` lands; rollback drops them silently) so a request that fails mid-handler can't leak an SSE frame for work that never persisted. The `emit_now()`/`emit_gameplay_now()` siblings are the immediate-publish escape hatches reserved for worker timer tasks where the reconciler retry loop makes leaked events self-healing — don't reach for them in routers. See `core/CLAUDE.md` Broadcast section.
