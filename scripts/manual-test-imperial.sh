...
TEMP_SECRET=$(echo "$TEMP_RESP" | jq_val "['player_secret']")
...
curl -sf -X POST "$BASE/games/$GAME_ID/start" \
  -H "X-Player-Id: $HOST_PLAYER_ID" -H "X-Player-Secret: $HOST_SECRET" > /dev/null
...
curl -sf -X POST "$BASE/games/$GAME_ID/questions/thermometer/$Q_ID/lock-in" \
  -H "X-Player-Id: $SEEKER_ID" -H "X-Player-Secret: $SEEKER_SECRET" > /dev/null
