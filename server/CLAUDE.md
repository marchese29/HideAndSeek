...
- **Broadcast — two layers**: Event broadcast is split between core (domain) and server (presentation):
  ...
  - **Mutation endpoints return 204**: `POST /questions/*` (ask, lock-in, answer, veto, abandon, randomize), `POST /hider-station`, `POST /expand-hiding-zone`, and `POST /end` return 204 with no body. State updates flow to clients exclusively through SSE events. Push notifications remain as supplementary wake-up alerts.
  ...
  - **Enriched hider location events**: `PlayerLocationEvent` carries three optional fields (`candidate_stations`, `not_in_zone`, `computed_answer`) for hider events only — all `None` for seeker events. Computed in `POST /location` handler after `create_location_update()` (the flushed row is visible in the same session). `candidate_stations` populated when election is `pending` or `ambiguous`, `not_in_zone` after election, `computed_answer` when an answerable question exists (uses `compute_hider_centroid()` as representative location). Same fields on `HiderGameStateResponse` for SSE reconnection (computed in `build_hider_game_state()`). Computation functions live in core: `compute_candidate_station_ids()`, `compute_not_in_zone()` in `logic/station.py`; `preview_answer()` in `logic/answer.py`.
  - **Proximity tier tracking**: On every seeker location update during seeking (after station election), the location handler calls `evaluate_proximity(game, player)` from core. If the tier changes, emits `ProximityEscalatedEvent` or `ProximityDeescalatedEvent` to the hider SSE channel and dispatches a hider-only push notification. `HiderGameStateResponse` includes `proximity_tier` for SSE reconnection hydration. Seekers receive no proximity information. See `core/logic/proximity.py` for the two-phase escalation/de-escalation algorithm.
