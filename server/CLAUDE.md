...
#              photo_submit_min, photo_review_sec
curl -s -X POST localhost:8000/games \
  -H "Content-Type: application/json" \
  -d "{\"map_id\": \"$MAP_ID\", \"name\": \"HostName\", \"size\": \"large\", \"photo_submit_min\": 2, \"photo_review_sec\": 15}"
...
# Hider submits an already-queued photo without re-uploading bytes (any hider can do
# this — queued state is fanned out via PhotoQueuedEvent):
curl -s -X POST localhost:8000/games/<game_id>/questions/<question_id>/photo \
  -H "X-Player-Id: $HIDER_PLAYER_ID" -H "X-Player-Secret: $HIDER_SECRET" \
  -F "submit=true"
# → 204. Same effects as upload+submit. 422 if no photo currently queued.
...
# Stamps Game.paused_at, adds PauseReason.host to active_pause_reasons.
# Reconciler skips overdue queries while paused; on resume, every live
# deadline (hiding_ends_at, found_claim_expires_at, open question_deadlines)
# shifts forward by the pause duration.
curl -s -X POST localhost:8000/games/<game_id>/pause \
  -H "X-Player-Id: $HOST_PLAYER_ID" -H "X-Player-Secret: $HOST_SECRET"
# → 204. Emits GameTimerPausedEvent on both SSE channels; push to all roles.
# → 403 if not host. 409 if not in hiding/seeking, or already host-paused.

# Host resumes (releases only the host pause reason).
curl -s -X POST localhost:8000/games/<game_id>/resume \
  -H "X-Player-Id: $HOST_PLAYER_ID" -H "X-Player-Secret: $HOST_SECRET"
# → 204. If host was the last reason: emits GameTimerResumedEvent (both channels)
#   with shifted deadlines; clock is live again. If other reasons remain
#   (e.g. photo_question_open): emits GameTimerPausedEvent with the reduced
#   list; clock stays paused. Push to all roles either way.
# → 403 if not host. 409 if not in hiding/seeking, or host not in
...
- **Game timers — reconciler-driven**: The `hideandseek-reconciler` process polls Postgres every second for overdue fire-times and enqueues the corresponding worker tasks for immediate execution. Authoritative fire-times live in dedicated future-deadline DB columns (`Game.hiding_ends_at`, `Question.deadline_at`, `Game.found_claim_expires_at`); the three non-photo queries also filter `Game.paused_at IS NULL`, so paused games are skipped without any extra revoke. Photo deadlines (`question.answerable_at + effective_photo_submit_min`, `photo_params.submitted_at + effective_photo_review_sec`) still resolve in Python until z32.5/m8r.nah moves them onto dedicated columns. Overdue queries in `hideandseek_core.logic.timers`. Router cancellation happens implicitly: when state advances (game ends, question answered, found-claim resolved), the reconciler's query filters on `Game.status` / `Question.status` / `<deadline> IS NOT NULL` skip those rows — no explicit `revoke()` needed. Task bodies retain their own status guards as a safety net for enqueue-during-execution races.
...
- **Broadcast — two layers**: Event broadcast is split between core (domain) and server (presentation):
  ...
  - **Emit call sites**: `routers/games.py` (join → `PlayerJoinedEvent`, remove → lobby `PlayerLeftEvent`/`HostChangedEvent` or gameplay `GamePlayerLeftEvent`/`GameHostChangedEvent`/`GameDissolvedEvent` via `RemovalResult`, patch → `PlayerUpdatedEvent` if lobby, start → `GameStartedEvent` + push, elect station → `StationElectionEvent`, expand hiding zone → `HidingZoneExpandedEvent` + push, end game → `GameEndedEvent` + push, host pause/resume → router calls `pause_game`/`resume_game` and `emit_gameplay()`s the returned `GameTimerPausedEvent` / `GameTimerResumedEvent` on both channels, plus push), `routers/location.py` (location report → `PlayerLocationEvent`, seeker location during seeking → `ProximityEscalatedEvent`/`ProximityDeescalatedEvent` + push to hiders), `routers/questions.py` (ask/lock-in/answer/veto/abandon/randomize → corresponding gameplay events), `routers/endgame.py` (found claim → `FoundClaimEvent` on both channels with `deadline_utc` + hider push, reject → `FoundClaimRejectedEvent` + seeker push, confirm → `GameEndedEvent` + push; schedules/revokes `found_claim:{game_id}` timer), `hideandseek_worker.tasks.game_timers` (phase transition → `PhaseChangedEvent` + `StationElectionEvent`, auto-answer → `HiderQuestionAnsweredEvent` + `SeekerQuestionAnsweredEvent` or `QuestionVetoedEvent`, auto-dismiss found claim → `FoundClaimExpiredEvent` + push — worker tasks call `emit_gameplay_now()` for immediate publish; reconciler retry covers any rollback-leaked events). `create_game_with_host()` does NOT emit (no SSE subscribers yet). Routers always go through the after-commit `emit_gameplay()` / `emit()` so a request that fails post-emit (e.g. broken `send_push.delay`) doesn't leak an SSE frame for work that rolled back — see `core/CLAUDE.md` Broadcast section.
  ...
  - **`SSEExposed` mixin** (`hideandseek.schemas.response`): marks `HiderGameStateResponse` and `SeekerGameStateResponse` for OpenAPI injection (they aren’t on REST routes). Sub-types (`GamePlayer`, `HiderActiveQuestion`, etc.) are pulled in automatically via `$defs` hoisting. `HiderActiveQuestion` carries three photo-only fields (`photo_subject`, `photo_queued_by`, `photo_uploaded_at`) so a hider can rehydrate a queued banner after SSE reconnect — inventory isn't on the hider snapshot, so subject can't be derived client-side. `SeekerActiveQuestion` does NOT mirror these — seekers derive `photo_subject` from inventory client-side.
  ...
  - **Mutation endpoints return 204**: `POST /questions/*` (ask, lock-in, answer, veto, abandon, randomize), `POST /hider-station`, `POST /expand-hiding-zone`, `POST /end`, `POST /pause`, `POST /resume`, and `POST /found` + `POST /found/confirm` + `POST /found/reject` all return 204 with no body. State updates flow to clients exclusively through SSE events. Push notifications remain as supplementary wake-up alerts.
  - **Host pause / resume** (`POST /games/{id}/pause`, `POST /games/{id}/resume` in `routers/games.py`): host-only; allowed during hiding or seeking (409 in lobby / finished / dissolved). 409 if already host-paused / not host-paused (router-side guard so double-taps surface clearly; the core primitive itself stays idempotent for callers like photo-questions that ref-count silently). The router calls `pause_game` / `resume_game` from `hideandseek_core.logic.pause` and dispatches `send_push.delay(..., PushEventType.game_timer_paused | game_timer_resumed, ...)` afterward — core only emits SSE per the standard layering. Resume releases only `PauseReason.host`; if other reasons remain (e.g. `photo_question_open`), the core primitive re-emits `GameTimerPausedEvent` with the reduced reason set instead of a resumed event, and the clock stays paused.
  ...
  - **Configurable game parameters**: `POST /games` accepts optional `size` (small/medium/large — not special), `hiding_time_min`, `base_question_delay_min`, `photo_submit_min`, and `photo_review_sec` overrides. Three-level fallback: request → map → code default (by size). `Game.size` is stored on the game (can differ from `GameMap.size`), used by `effective_hiding_zone_radius_m()`. The hiding/question fields resolve at create time (stored as non-null ints); the photo fields stay nullable on `Game` and resolve lazily via `effective_photo_submit_min(game)` / `effective_photo_review_sec(game)`. `MapSummary` and `MapDetail` expose nullable `default_hiding_time_min`, `default_base_question_delay_min`, `default_photo_submit_min`, and `default_photo_review_sec` for client-side default computation. Inventory stays map-based (not affected by game size override).
  ...
  - **Game-timer pause snapshot fields**: Both `HiderGameStateResponse` and `SeekerGameStateResponse` carry six pause-aware fields — `paused: bool`, `paused_at: datetime | None`, `active_pause_reasons: list[PauseReason]`, `seeking_pause_accumulated_sec: int`, `hiding_ends_at: datetime | None`, `found_claim_expires_at: datetime | None` — populated as pure passthroughs from `Game.*` in `build_hider_game_state` / `build_seeker_game_state`. Backgrounded clients that miss `game_timer_paused` / `game_timer_resumed` SSE events recover full pause state from the next snapshot. The dual-channel events (emitted by `core/logic/pause.py`) carry shifted deadlines on resume; the snapshot is the authoritative recovery path on reconnect.
