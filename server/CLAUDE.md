...
- **Broadcast — two layers**: Event broadcast is split between core (domain) and server (presentation):
  ...
  - **`GameplayEventType` enum** (`hideandseek_models.types`): `game_state` (initial snapshot), `player_location`, `question_asked`, `question_answerable`, `question_answered`, `question_vetoed`, `question_abandoned`, `phase_changed`, `station_election`, `player_left`, `host_changed`, `game_dissolved`, `hiding_zone_expanded`.
  ...
  - **Mutation endpoints return 204**: `POST /questions/*` (ask, lock-in, answer, veto, abandon, randomize), `POST /hider-station`, and `POST /expand-hiding-zone` return 204 with no body. State updates flow to clients exclusively through SSE events. Push notifications remain as supplementary wake-up alerts.
  ...
  - **Enriched hider location events**: `PlayerLocationEvent` carries three optional fields (`candidate_stations`, `not_in_zone`, `computed_answer`) for hider events only — all `None` for seeker events. Computed in `POST /location` handler after `create_location_update()` (the flushed row is visible in the same session). `candidate_stations` populated during hiding phase with `pending` election, `not_in_zone` after election, `computed_answer` when an answerable question exists (uses `compute_hider_centroid()` as representative location). Same fields on `HiderGameStateResponse` for SSE reconnection (computed in `build_hider_game_state()`). Computation functions live in core: `compute_candidate_station_ids()`, `compute_not_in_zone()` in `logic/station.py`; `preview_answer()` in `logic/answer.py`.
