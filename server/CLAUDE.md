...
- `broadcast/` in server handles lobby events only (Pydantic response serialization, SSE subscription streams). Gameplay event dataclasses and `emit_gameplay()` live in `hideandseek_core.broadcast` — shared by routers and tasks.
...
- **Three-package architecture**: `hideandseek-models` (ORM) ← `hideandseek-core` (business logic, queries, DB, geo, push, redis, celery, gameplay broadcast) ← `hideandseek` (server: routers, schemas, lobby broadcast, tasks). Core owns gameplay event production and Redis publishing. Server owns lobby events (Pydantic serialization), SSE subscriptions, and HTTP routing.
...
- **Broadcast — two layers**: Event broadcast is split between core (domain) and server (presentation):
  - **Core** (`hideandseek_core.broadcast`): Gameplay event dataclasses (`events.py`) + `emit_gameplay()` (`emit.py`). Also provides `publish_sse()` (low-level Redis publish) and channel name helpers (`lobby_channel`, `hider_channel`, `seeker_channel`). Both routers and tasks import `emit_gameplay` from here.
