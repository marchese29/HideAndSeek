...
- **Geometry — three layers**: Geometry flows through three representations:
  ...
  - **Database** — PostGIS spatial columns with two column types in `hideandseek_models.geo_types`:
...
- SQLAlchemy 2.0 declarative ORM (`Mapped[]` + `mapped_column()`) for all table models. All models inherit from `Base` in `hideandseek_models.base`.
