...
- **Subject labels**: `src/utils/photoSubjects.ts` exports `PHOTO_SUBJECT_LABELS` (full enum → human string) and `photoSubjectLabel(subject)` for nullable lookup. The 18 subject strings are mobile-only — the server transmits raw `PhotoSubject` enum values and the label is resolved client-side. Used by ParamPicker, SeekerBanner, QuestionHistoryRow, and PhotoViewerModal.
- **Belt + picker**: `QuestionTypeBar` renders the Photo button (`camera` icon, color from `questionColors.ts:photo`); inventory filtering already gates visibility when no photo slots remain. `ParamPicker.formatSlotLabel` has a leading photo branch — the pill text shows the subject label. `useQuestionSelection.buildPreview` builds a `PhotoEventParams` preview from the slot's `photo_subject`.
