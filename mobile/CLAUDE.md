...
- **`photo_submitted`** / **`photo_rejected`**: Photo lifecycle — `applyPhotoSubmitted()` patches the active question's `status` (`submitted`), `submitted_at`, `is_null_answer`, `review_deadline`, and clears `question_deadline`. `applyPhotoRejected()` reverts `status` to `answerable`, sets `question_deadline = new_submit_deadline`, and clears the submitted-state fields. Both guard on `active_question.question_id === delta.question_id` so a stale event doesn't touch a different active question after a fast re-ask. Both channels carry these events; both stores apply them so cycle 7's hider UI gets the same plumbing for free.
...
Hider UI lives in cycle 7 (HideAndSeek-z32.7); this section covers the seeker side only.
