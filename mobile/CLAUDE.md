...
src/
  ...
  components/                  # Reusable UI components
    ...
    more-modal/                # More modal — bottom sheet for game meta-actions
      ...
      MoreMenu.tsx             # Preferences-style menu list (Stats, Preferences, Leave, Pause Game, End Game, Kick) — Pause Game gated on isHost && gameIsActive && !isHostPaused, fires Alert.alert via MoreModal's onSelect
      ...
      gameActions.ts           # Extracted async functions: doLeave, doEndGame, doKick, doPause, doResume
...
- **Zustand — GameplayStore** (`src/stores/gameplayStore.ts`) — **dynamic** gameplay state hydrated from SSE. Not persisted — rebuilt from the SSE snapshot on every connection. Discriminated union: `{ status: 'connecting' }` or `{ status: 'connected', role, state }`. The `hydrate()` action replaces state from a `game_state` SSE event; `reset()` reverts to connecting. State includes `host_player_id` for leave/host-transfer UI. Also holds phone-local `photoViewer: { questionId, subject, submittedBy, submittedAt } | null` driving `<PhotoViewerModal />` mounted at the gameplay screen root — `openPhotoViewer(...)` / `closePhotoViewer()` actions; preserved across SSE reconnects (cleared on `reset()`). Does NOT include static map data (boundary, stops, routes, timing) — that's in TanStack Query via `useGameInfo`. Used by `useGameplayEvents` hook.
...
- **`photo_submitted`** / **`photo_rejected`**: Photo lifecycle — `applyPhotoSubmitted()` patches the active question's `status` (`submitted`), `submitted_at`, `is_null_answer`, `review_deadline`, and clears `question_deadline`. `applyPhotoRejected()` reverts `status` to `answerable`, sets `question_deadline = new_submit_deadline`, and clears the submitted-state fields. Both clear `photo_queued_by` + `photo_uploaded_at` (hider snapshot only). Both guard on `active_question.question_id === delta.question_id` so a stale event doesn't touch a different active question after a fast re-ask. Both channels carry these events; both stores apply them. The hider also gets toasts: `"Seeker accepted your photo."` (info) on `question_answered` for photo, `"Seeker rejected your photo — try again."` (warning) on `photo_rejected`.
- **`photo_queued`** / **`photo_unqueued`** (hider channel only): `applyPhotoQueued()` patches `photo_queued_by` + `photo_uploaded_at` on the active hider question; `applyPhotoUnqueued()` clears them. Both guard on question id match. Used to fan out queued state to all hiders so any can Submit Now without re-uploading.
...
## Photo Questions (Hider UI)

- **Dependencies**: `expo-image-picker` (`~55.x`) for the picker, `expo-image-manipulator` (`~55.x`) for HEIC→JPEG normalization. Plugin block in `app.config.ts` sets `photosPermission`; iOS `infoPlist.NSPhotoLibraryUsageDescription` mirrors it. Adding/changing either requires a native rebuild.
- **API helpers** (`src/api/questions.ts`): `uploadPhoto`, `submitQueuedPhoto`, `submitNullPhoto`, `unqueuePhoto`. All four return `PhotoApiResult = { ok: true } | { ok: false; status; detail }` so callers can distinguish 409 (raced submission) from other failures. They use raw `fetch` (openapi-fetch can't model `FormData`); auth via `authHeader()`. **Do not set `Content-Type` for multipart** — RN sets the boundary itself. Existing `acceptPhoto`/`rejectPhoto` stay boolean since seekers don't need 409 visibility.
- **Banner branches** (`HiderBanner.tsx`): four photo states inserted before the generic answerable render (photo never falls through). Subject text resolves via three-tier fallback: `parameters.subject` (delta) → `photo_subject` (snapshot) → `'Photo'`.
  - **A — Asked** (`status==='answerable'`, no queue, no local pick): subject + countdown + Pick Photo (primary) + Null (cancel icon) + Power-Up.
  - **B — Pick preview** (local `localPick` set): thumbnail + subject (flex-grown, right-justifies the buttons) + Submit Now (send) + Queue (timer-sand) + Cancel (close). No Replace — Cancel returns to Asked which has Pick Photo.
  - **C — Queued** (`photo_queued_by != null`): cache-busted thumbnail (tap → viewer) + subject + "Queued by {name|you}" sub-label + countdown + Submit Now + Replace + Unqueue. Power-Up + Null intentionally omitted (Unqueue → Asked exposes both). Thumbnail URL appends `?v=<photo_uploaded_at>` because RN `<Image>` caches by URL — without it Replace would show stale bytes.
  - **D — Submitted** (`status==='submitted'`): thumbnail (or "null" placeholder) + "Submitted — waiting for review..." + review countdown. No actions.
- **Local pick lifecycle**: `localPick` is component-local `useState`. A `useEffect` clears it whenever `question_id` / `status` / `photo_queued_by` changes — covers SSE-driven transitions (queue lands, reject loops back, submit lands).
- **409 handling**: shared `handlePhotoResult` clears any local pick and pushes an info toast (`"Another hider already submitted."`). Other failures show `Alert.alert('Upload Failed', detail)`. Unqueue silently swallows 409 (auto-submit may have raced).
- **Permissions**: `ensurePhotoPermission()` checks status, requests if `canAskAgain`, otherwise prompts the user with an Alert + "Open Settings" via `Linking.openSettings()`.
- **MIME normalization**: server only accepts `image/jpeg` / `image/png` with matching magic bytes. `normalizeAsset()` checks the picker's `mimeType` (or filename extension) — JPEG/PNG pass through as `{ uri, name, type }`; anything else (notably iOS HEIC/HEIF, which the simulator and modern iPhones produce by default) is transcoded via `ImageManipulator.manipulateAsync(..., { format: SaveFormat.JPEG, compress: 0.9 })`. RN `FormData` requires `{ uri, name, type }` — cast to `Blob` to satisfy DOM-typed `append()`.
- **Photo viewer modal opens** (`onOpenViewer`): for queued state, `submittedBy = photo_queued_by`, `submittedAt = photo_uploaded_at`. Reuses the same `openPhotoViewer` action seekers use; existing modal already handles the null-submitter case.
- **Snapshot fields** on `HiderActiveQuestion`: `photo_subject` (denormalized so the banner labels right after SSE reconnect — inventory isn't on the hider snapshot), `photo_queued_by`, `photo_uploaded_at`. `setActiveQuestion` initializes `photo_subject` from `delta.parameters.subject` when applicable, leaves queued fields null. `applyPhotoSubmitted` / `applyPhotoRejected` clear queued fields. `applyPhotoQueued` / `applyPhotoUnqueued` patch them.

## Photo Questions (Seeker UI)

- **Subject labels**: `src/utils/photoSubjects.ts` exports `PHOTO_SUBJECT_LABELS` (full enum → human string), `PHOTO_SUBJECT_SHORT_LABELS`, `PHOTO_SUBJECT_ICONS` (MaterialCommunityIcons name per subject), and accessor helpers `photoSubjectLabel` / `photoSubjectShortLabel` / `photoSubjectIcon`. The 18 subject strings are mobile-only — the server transmits raw `PhotoSubject` enum values and the label is resolved client-side. Used by ParamPicker, SeekerBanner, QuestionHistoryRow, PhotoViewerModal, and the picker modal's PhotoFamily.
- **Picker modal (muo.1)**: photo questions ask via `QuestionPickerModal` (`src/components/question-picker-modal/`), not the in-belt `ParamPicker`. `UtilityBelt` intercepts the type bar's `onSelectType('photo')` to set local `pickerVisible = true` instead of dispatching to `useQuestionSelection` — the belt stays in `step: 'type'` so dismissing returns to the type-icon takeover. Selection is component-scoped state on `UtilityBelt` (`pickerSubject`), so it persists across modal opens within a game and clears when the gameplay screen unmounts on game end. The modal subscribes to `gameplayStore` via individual primitive selectors for live-reactive Submit gating: Submit disables instantly when an active question lands and re-enables when it terminates, without closing or re-mounting. First-touch accidental-submit guard: a confirmation Alert fires only when Submit is the very first picker touch after open. Submit POSTs `/games/{game_id}/questions/photo` with `slot_index` resolved from inventory by subject; success closes the modal, errors surface as Alert. Until muo.4, only photo routes through the modal — non-photo types still flow through `useQuestionSelection`'s `param`/`custom` steps and the in-belt `ParamPicker`.
- **Belt + picker (legacy, non-photo)**: `QuestionTypeBar` renders the Photo button (`camera` icon, color from `questionColors.ts:photo`); inventory filtering already gates visibility when no photo slots remain. `ParamPicker.formatSlotLabel` has a leading photo branch — the pill text shows the subject label (kept in place until muo.4 deletes the in-belt path). `useQuestionSelection.buildPreview` builds a `PhotoEventParams` preview from the slot's `photo_subject` (unreachable for photo in muo.1; preserved for muo.4 teardown).
- **Ask path**: `SeekerBanner.onAsk` branches on `previewQuestion.question_type === 'photo'` and POSTs `/games/{game_id}/questions/photo` with `{ slot_index, location }` (no `custom_distance`). All other types share the existing `ASK_PATHS` map.
- **Banner — answerable**: `SeekerBanner` renders a camera-icon row with `"<subject> — waiting for photo..."`, a `BannerCountdown` against `question_deadline` (the photo submit window — see core's `effective_question_deadline`), and an Abandon X.
- **Banner — submitted**: same row swaps in either a tappable thumbnail (`<Image>` with `authHeader()`, native HTTP cache honors `Cache-Control: private, max-age=3600`) or, when `is_null_answer === true`, a placeholder card with the literal word **"null"** (italic). `null` is a real answer ("can't get this photo") — never paraphrased as "gave up" / "surrender" / "no photo". Three icon buttons: Accept (✓), Reject (↻), Abandon (✗). Accept/Reject use confirmation `Alert.alert`s; both endpoints return 204 and UI transitions follow from SSE (`question_answered` for accept, `photo_rejected` for reject).
- **Photo viewer modal**: `src/components/PhotoViewerModal.tsx` is mounted once at the gameplay screen root, driven by `gameplayStore.photoViewer` (`{ questionId, subject, submittedBy, submittedAt } | null`). Opens via `openPhotoViewer(...)` from any photo-tap entry point (banner thumbnail, history thumbnail). Pinch-zoom uses `<ScrollView maximumZoomScale={4} pinchGestureEnabled>` — no `react-native-gesture-handler` / `react-native-reanimated` dependency. Submitter name resolves from the gameplay store roster.
- **Question history row**: `QuestionHistoryRow` accepts an optional `gameId` prop. When set + photo + answered + non-null, the right column renders a small (~36×36) tappable thumbnail that opens the viewer (own `Pressable`, independent of the row's outer `onPress`). When answer is `'null'`, renders the literal word **"null"** (no thumbnail, no tap). `paramSummary` resolves photo questions to their subject label. Cutoff modal callers omit `gameId`, so cutoff rows render without a tappable thumbnail (the row's outer `Pressable` handles selection).
- **History modals**: both `HiderQuestionHistoryModal` and `SeekerQuestionHistoryModal` thread `gameId` through to `QuestionHistoryRow`. Photo bytes are accessible to both roles for terminal questions.
- **API helpers**: `src/api/questions.ts` adds `acceptPhoto(gameId, questionId)` and `rejectPhoto(gameId, questionId)`. Ask uses `api.POST` directly in `SeekerBanner` (consistent with the existing pattern).
...
- **PauseModal** (`src/components/PauseModal.tsx`): non-dismissable `pageSheet` Modal (`onRequestClose` no-op, mirrors `FoundClaimModal`). When `host` is in `active_pause_reasons` AND the viewer is the host, renders a bottom-pinned buttons row: **End Game** (left, destructive red `#E74C3C` → `doEndGame`) and **Resume** (right, primary blue `#3498DB` → `doResume`). Local `submitting` `useState` gates both buttons; failures surface as `Alert.alert`. Non-host viewers (or non-host pause categories like `rest_period`) see only the title + body. Resume dismissal is SSE-driven: if no other reasons remain, `game_timer_resumed` clears `paused`; if another reason is still active (e.g. `photo_question_open`), `game_timer_paused` re-emits with reduced reasons → `isUniversalCategory` returns false → modal hides and `<PauseBanner />` takes over. The rest-period countdown lands with a future epic.
...
- **Host pause trigger**: `MoreModal` shows a "Pause Game" item between Leave and End Game when the viewer is the host, the game is `hiding`/`seeking`, and `host` is NOT already in `active_pause_reasons` (gated via `MoreMenu`'s new `isHostPaused` prop, derived from `usePaused()` in `MoreModal`). Selecting it fires `Alert.alert('Pause Game?', …, [Cancel, Pause])`; on confirm, `MoreModal` closes and `doPause(gameId)` POSTs `/games/{id}/pause`. The SSE-driven `<PauseModal />` is the surface that resolves the pause (Resume / End Game). Photo-questions trigger (HideAndSeek-nah) is the next consumer; until that lands, exercise stacked-pause scenarios by invoking `pause_game(reason=photo_question_open)` directly via `docker compose exec api uv run python -c "..."`.
