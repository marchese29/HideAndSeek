...
src/hideandseek_core/
  ...
  conventions.py       # Metric/imperial conversion, default inventory (now incl. 'photos' key
                       ...
                       #   submit window for photo+answerable, otherwise reads
                       #   Question.deadline_at (the pause-shifted future-deadline column)
...
- `emit_gameplay(event)` — **after-commit-safe**. Buffers the event on `Session.info["pending_gameplay_emits"]`; a SQLAlchemy `after_commit` listener (registered at module import) flushes the buffer to Redis once the active transaction lands. `after_rollback` drops the buffer silently. Use this from any code path running inside `session_dependency` (router) or `session_scope` (background task / script). Routing/serialization happens at flush time via the private `_publish_gameplay()` helper.
- `emit_gameplay_now(event)` — immediate-publish escape hatch. Routes and `PUBLISH`es synchronously, no transactional gating. Reserved for worker timer task bodies where the reconciler's retry loop makes leaked events self-healing (a published-then-rolled-back event becomes a re-enqueued task on the next reconciler tick, and clients converge). **Don't use this in routers** — the post-emit transaction can fail and strand clients on stale state. Tracked at HideAndSeek-2wx: should we collapse worker emits onto the after-commit path too.

The same pair lives in `server/broadcast/emit.py` for lobby events: `emit()` (after-commit) and `emit_now()` (immediate; no current callers, kept for symmetry).
...
- `queue_photo(question, player, object_key)` — persist a queued (not yet submitted) photo and stamp `params.queued_at` / `params.queued_by`. Caller has already uploaded bytes to S3; overwriting an existing key is intentional (the previous S3 object is retained — photos kept forever per design §10). The persisted `queued_at` is what `PhotoQueuedEvent.uploaded_at` and the hider snapshot's `photo_uploaded_at` share, so the mobile cache-bust key matches across SSE event and reconnect snapshot.
- `clear_queued_photo(question)` — zero out the queued key, `is_null_answer` flag, and `queued_at` / `queued_by` (DELETE endpoint).
- `submit_photo_question(question, player, *, is_null_answer)` — flip status to `submitted`, stamp `submitted_at` / `submitted_by`, clear `queued_at` / `queued_by`. For `is_null_answer=True` also clears any queued key; otherwise the caller must have queued a photo first.
- `auto_submit_photo(question)` — reconciler-driven variant when the submit window expires with content queued; `submitted_by` stays NULL so audit trails distinguish auto from manual. Also clears `queued_at` / `queued_by`.
...
- `reject_photo(question, *, reviewer_id)` — flips `submitted` back to `answerable`; nulls submission state, the queued photo key, and `queued_at` / `queued_by`; resets `answerable_at` so the reconciler's submit-window timer restarts.

Reconciler queries in `logic/timers.py`: the three non-photo queries (`find_overdue_hiding_games`, `find_overdue_answerable_questions`, `find_overdue_found_claims`) read directly from the future-deadline columns (`Game.hiding_ends_at`, `Question.deadline_at`, `Game.found_claim_expires_at`) and filter `Game.paused_at IS NULL` so paused games are naturally skipped. `find_overdue_answerable_questions` excludes `QuestionType.photo` (those are auto-answered through the dispatcher and would otherwise misfire); `find_overdue_photo_submissions` and `find_overdue_photo_reviews` cover the two photo-specific deadlines (Python-side filter using `effective_photo_submit_min` / `effective_photo_review_sec`) and stay on the start-anchor shape until z32.5/m8r.nah introduces `PhotoQuestionParams.{submit,review}_deadline_at` and the matching `paused_at` filter. Reused helpers in `logic/answer.py` were renamed from underscore-prefixed (`accumulate_exclusion`, `log_question_answered`) so cross-module callers don't reach for private API.
...
- `pause_game(game, reason) -> GameplayEvent | None` — adds `reason` to `game.active_pause_reasons`; stamps `paused_at` if the set was empty. Idempotent on `reason` (duplicate add returns `None`). Otherwise returns a `GameTimerPausedEvent` for the caller to emit. The router (or worker task) calls `emit_gameplay(event)` from the boundary that owns the transaction — logic functions don't reach into the broadcast layer.
- `resume_game(game, reason) -> GameplayEvent | None` — removes `reason` from the set. Idempotent (releasing a non-active reason returns `None`). If reasons remain, returns a `GameTimerPausedEvent` with the reduced list (clock stays paused). If the set becomes empty, shifts every live deadline forward by `now - paused_at`, increments `seeking_pause_accumulated_sec`, clears `paused_at`, and returns a `GameTimerResumedEvent` carrying every shifted deadline.
...
Today's resume fan-out covers `Game.hiding_ends_at`, `Game.found_claim_expires_at`, and open `Question.deadline_at` (queried via `get_open_questions_with_deadlines`). Photo deadlines (`PhotoQuestionParams.submit_deadline_at` / `review_deadline_at`) join in m8r.nah once those columns exist. Both events are dual-channel (hider + seeker SSE). Both SSE emit and push dispatch are owned by the calling router (m8r.nah for photo, m8r.8 for host endpoints) — `pause_game` / `resume_game` return the `GameplayEvent | None` for the boundary to emit, keeping logic functions out of the broadcast layer.
